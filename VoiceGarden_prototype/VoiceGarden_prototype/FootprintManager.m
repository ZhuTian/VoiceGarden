...
@implementation FootprintManager
...
{
    FootprintManager* manager = [[[self class] alloc] init];
...
-(void)updateOpacity:(ccTime)dt
{
    ...
    for(int i= 0; i < [footArray count]; i++)
    {
        ...
        tempFoot.position = ccp(tempFoot.position.x - scrollSpeed * dt, tempFoot.position.y);
